...
`.github/workflows/release.yml` は `v*` 形式のタグ push、または
`workflow_dispatch` で macOS と Windows 向けの Tauri バンドルを生成し、
GitHub Release の draft に成果物を添付する。

## トリガー

| トリガー | 動作 |
| --- | --- |
| `git tag v0.1.0 && git push origin v0.1.0` | タグ名で draft release を作成し artifact を添付 |
| Actions UI -> release -> Run workflow | `release_tag` 入力で任意のタグ名でテスト実行 |
...
1. Secrets を未設定のまま試したい場合:
   ...
   3. アーティファクトをダウンロードして起動する
2. Secrets 設定後:
   1. 上記と同様にタグ push して artifact をダウンロード
...
- macOS の Universal バイナリは matrix で 2 ターゲットを別ジョブとして生成している。
  単一の `.app` に統合する場合は `tauri-action` の `args` を
  `--target universal-apple-darwin` に変更し、Rust toolchain に
  `aarch64-apple-darwin` と `x86_64-apple-darwin` を両方追加すること。
- リリースは `releaseDraft: true` で生成されるので、最終公開はユーザー操作で行う。
- `ci.yml` の `clippy -D warnings` は新規 lint 違反でビルドを落とすため、
  main マージ前の `cargo clippy --all-targets -- -D warnings` 実行を推奨。
